...
struct Battery {
  enum Status : int64_t { charging = 1, discharging = -1, unknown = 0, full = 2 };
  ...
  explicit Battery(std::string_view config)
      : status(initStatus(config)), voltageMinDesign(getUInt64_tAfterKey(config, "POWER_SUPPLY_VOLTAGE_MIN_DESIGN=")),
        voltageNow(getUInt64_tAfterKey(config, "POWER_SUPPLY_VOLTAGE_NOW=")),
        currentNow(getUInt64_tAfterKey(config, "POWER_SUPPLY_CURRENT_NOW=")),
        chargeFullDesign(getUInt64_tAfterKey(config, "POWER_SUPPLY_CHARGE_FULL_DESIGN=")),
        chargeFull(getUInt64_tAfterKey(config, "POWER_SUPPLY_CHARGE_FULL=")),
        chargeNow(getUInt64_tAfterKey(config, "POWER_SUPPLY_CHARGE_NOW=")),
        capacity(getUInt64_tAfterKey(config, "POWER_SUPPLY_CAPACITY=")) {}

  // returns the power draw in watts
  [[nodiscard]] double getPowerDraw() const { return (double)voltageNow * 1e-6 * (double)currentNow * 1e-6; }
